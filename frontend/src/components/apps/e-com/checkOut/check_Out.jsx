...
import { Grid, Card, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, Typography, Container, Button, Modal, Backdrop, CardContent, FormControl, Fade } from "@material-ui/core";
...
export default function CheckOut() {
    ...
    const subtotal = checkOutItems.reduce((acc, item) => acc + parseFloat(item.product.price) * item.quantity, 0);
    ...
    const placeFinalOrder = () => {
        ...
        const items = checkOutItems
            .map((item, index) => {
                return `${index + 1}. Product Name: ${item.product.product_name}, Description: ${item.product.description}, Price: $${item.product.price}, Quantity: ${item.quantity}, Total: $${(
        ...
        // Total price
        const totalPriceInfo = `Total Price: ${totalPrice.toFixed(2)}, Delivery Fee: $${deliveryFee.toFixed(2)}, Total Amount: $${(totalPrice + deliveryFee).toFixed(2)}`;
        ...
        const orderDetailsJSONFormatter = {
            ...
            shipping: {
                details: {
                    orderedDate: new Date(new Date().setDate(new Date().getDate())).toDateString(),
                    expectedDeliveryDate: new Date(new Date().setDate(new Date().getDate() + 4)).toDateString(),
            ...
            items: checkOutItems.map((item, index) => {
                return {
                    ...
                    total: (parseFloat(item.product.price) * parseInt(item.quantity)).toFixed(2),
        ...
        const buyer_delivery_address = document.getElementById("buyerDefaultAdress").textContent;
        ...
        const product_total_price = document.getElementById("totalPrice").textContent;
        ...
        const convertedOrderDetails = {
            ...
            product_category_name: checkOutItems.map((item) => item.product.category_name).join(","),
            product_category: checkOutItems.map((item) => item.product.category).join(","),
                ...
                <Grid container spacing={3} className="hide-print">
                    ...
                    <Grid item md={3}>
                        <FormControl component="fieldset">
                            ...
                            <Button variant="contained" color={paymentMethod === "cashOnDelivery" ? "primary" : "default"} disabled={paymentMethod !== "cashOnDelivery"} onClick={() => handlePaymentMethodChange("cashOnDelivery")}>
                                Cash on Delivery
                            </Button>
                            <Button variant="contained" color={paymentMethod === "creditCard" ? "primary" : "default"} disabled={paymentMethod !== "creditCard"} onClick={() => handlePaymentMethodChange("creditCard")}>
                                Credit Card
                            </Button>
                            <Button variant="contained" color={paymentMethod === "Khalti" ? "primary" : "default"} disabled={paymentMethod !== "Khalti"} onClick={() => handlePaymentMethodChange("Khalti")}>
                                Khalti
                            </Button>
                            <Button variant="contained" color={paymentMethod === "e-sewa" ? "primary" : "default"} disabled={paymentMethod !== "e-sewa"} onClick={() => handlePaymentMethodChange("e-sewa")}>
                                e-sewa
                            </Button>
                        </FormControl>
                    </Grid>
                            ...
                            <Card>
                                <CardContent>
                                    ...
                                    <Grid>
                                        ...
                                        <p>
                                            Ordered Date:
                                            <b> {new Date(new Date().setDate(new Date().getDate())).toDateString()} </b>
                                        </p>

                                        <p>
                                            Expected Delivery Date:
                                            <b> {new Date(new Date().setDate(new Date().getDate() + 4)).toDateString()}</b>
                                        </p>
                                                        ...
                                                        if (!isNaN(price) && !isNaN(quantity)) {
                                                            ...
                                                            return (
                                                                <TableRow key={index}>
                                                                    ...
                                                                    <TableCell>{item.product.description.length > 50 ? item.product.description.substring(0, 50) + "..." : item.product.description}</TableCell>
                                    ...
                                    <br />
                                    e-com | ©CopyRight | Online Receipt Generated | Date: {new Date(new Date().setDate(new Date().getDate())).toDateString()}{" "}
                                </CardContent>
                            </Card>
                            <br />
                            <Button variant="outlined" color="secondary" className="hide-print" onClick={placeFinalOrder}>
                                Confirm Order
                            </Button>
                        </Grid>
                    </div>
